...
The motivation of the text is to provide a story mode for the lecture note \cite{zqhNA}, explaning the motivation and the background. We should firstly consider such a model problem.
\begin{equation}
	\left\{
		\begin{array}{l}
		-u''(x)=f(x),\;x\in(0,1),\\
...
\begin{equation}
	...
	\end{bmatrix}.
\end{equation}
It maps $(\cdots,a,\cdots)^T\in\Omega^h$ to $(\cdots,\frac{a}{4},\frac{a}{2},\frac{a}{4},\cdots)^T\in\Omega^{2h}$. Conversely, we have the prolongation operator $I_{2h}^h:\Omega^{2h}\to \Omega^h$ defined by
\begin{equation}
	...
	\end{bmatrix}.
\end{equation}
We always call it the linear interpolation operator since it maps $(\cdots,0,a,b,0,\cdots)^T\in\Omega^{2h}$ to $(\cdots,0,\frac{a}{2},a,\frac{a+b}{2},b,\frac{b}{2},0,\cdots)^T\in\Omega^{h}$.

In the end of the section, we should illustrate the conjugating principle
\begin{equation}
	I_{2h}^h=c\left(I_h^{2h}\right)^T,\quad \text{here}\; c=2,
\end{equation}
and the Garlerkin condition
\begin{equation}
...
Consider the invariant subspaces of $G$. Recall $\mathbf{w}_k\;(k=1,...,n-1)$ in (8) and we write $k'=n-k$. We call the vector pair $\mathbf{w}_k$ and $\mathbf{w}_{k'}$ to be the complementary modes for each $k\in[1,\frac{n}{2})$. One can verify that
\begin{equation}
	G\begin{bmatrix}
		\mathbf{w}_k\\
		\mathbf{w}_{k'}
...
Recall how we derive a matrix norm from a vector norm:
\begin{equation}
	||B||=\max_{||\mathbf{u}||\leq 1} ||B\mathbf{u}||.
\end{equation}
The derived matrix norm naturally satisfies the condition
\begin{equation}
	||B\mathbf{u}||\leq ||B||\cdot ||\mathbf{u}||.
\end{equation}
Also we should see the relationship between the matrix norm and the spectral radius as the following theorem.

\textbf{Theorem 1.1.} Let $||\cdot||$ be an arbitrary matrix norm. Its value is always no less than the spectral radius, i.e.
\begin{equation}
	\rho(B)\leq ||B||,\quad \forall B.
\end{equation}
Morever, when $||\cdot||=||\cdot||_2$ and the matrix $B$ is real symmetric, we have
\begin{equation}
	\rho(B)= ||B||_2.
\end{equation}

Let's return to the energy norm. Clearly we can derive a matrix norm from the energy norm of $A$. What's the relationship between $||B||_A$ and $\rho(B)$ if $B$ is real symmetric? Here we will give an result. If you don't care about the proof, jump to the end of this section to see the result directly.

We suppose $A$ is real symmetric and positive definite. Then we have the orthogonal diagonal decomposition
\begin{equation}
	A=Q^T\Lambda Q
\end{equation}
where $\Lambda=\text{diag}(\lambda_1,...,\lambda_n)$ and $\lambda_i$s are eigenvalues of $A$. They are all positive since $A$ is positive definite. Let $\mathbf{u}$ be an arbitrary vector and $\mathbf{v}=Q\mathbf{u}$. Consequently,
\begin{equation}
	||\mathbf{u}||_A^2=u^TAu=v^T\Lambda v=\sum_{i=1}^n \lambda_iv_i^2 \leq \rho(\Lambda)||\mathbf{v}||_2^2=\rho(A)||\mathbf{u}||_2^2.
\end{equation}
That is
\begin{equation}
	||\mathbf{u}||_A\leq \sqrt{\rho(A)}||\mathbf{u}||_2.
\end{equation}
Similarly if we denote $\sigma(A):=\lambda_\text{min}(A)=\min_i |\lambda_i|$, we have
\begin{equation}
	||\mathbf{u}||_A\geq \sqrt{\sigma(A)}||\mathbf{u}||_2.
\end{equation}
Hence for an arbitrary real symmetric matrix $B$, we have
\begin{align}
	||B||_A&=\max_{||\mathbf{u}||_A\leq 1}||B\mathbf{u}||_A\leq \sqrt{\rho(A)} \max_{||\mathbf{u}||_A\leq 1}||B\mathbf{u}||_2 \nonumber \\
	 &\leq \sqrt{\rho(A)} \max_{||\mathbf{u}||_2\leq \sqrt{\sigma(A)}^{-1}}||B\mathbf{u}||_2=\sqrt{\frac{\rho(A)}{\sigma(A)}}\rho(B).
\end{align}

As an end of the section, we summarize the discussion as the following theorem.

\textbf{Theorem 1.2.} If $A$ is real symmetric and positive definite, $B$ is real symmetric, then the energy norm satisfies
\begin{equation}
	\rho(B)\leq ||B||_A\leq \sqrt{\frac{\rho(A)}{\sigma(A)}}\rho(B).
\end{equation}
...
\end{align}
Since we have $\rho(G)=\frac{1}{9}$ as $\nu_1=\nu_2=1$, hence we can reduce the initial error to $Kp^2$ after acting a two-grid operator. This is illustrated in \cite{BriggsMultigrid}. It looks suspect since it cannot be strictly proved by theorem 1.2. In fact, we have proved the following theorem by induction.

\textbf{Theorem 1.3.} If The interpolation operator is of order at least $2$. Then the algebraic error of FMG-cycle satisfies
\begin{equation}
