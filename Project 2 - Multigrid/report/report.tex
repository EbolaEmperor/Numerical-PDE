...
采用full-weighting + 二次插值，每两步迭代的误差分布如下图。
\begin{figure}[H]
  \centering
  \begin{minipage}[t]{0.22\linewidth}
      \centering
...
在这一节中，我们以齐次Dirichlet边值问题为例给出理论分析，此时的离散方程为$A\mathbf{u}=\mathbf{f}$，系数矩阵$A\in\mathbb{R}^{n^2\times n^2}$是分块矩阵，
\begin{equation}
...
\begin{lemma}
  式(4.4)中矩阵$A$的加权Jacobi迭代矩阵为：
  \begin{equation}
    T_\omega=(1-\omega)I+\omega D^{-1}(L+U)=I-\frac{\omega h^2}{4}A.
  \end{equation}

  $T_\omega$的特征向量与$A$一样，对应的特征值为：
  \begin{equation}
...
\begin{proposition}
  取$\nu_1=\nu_2=1$，有$\rho(TG)<0.25$，因此能Two-Grid校正能够快速收敛。
\end{proposition}
...
因此，我们只需对每个$i,j\in[0,\frac{n}{2})$，求出相应的$C=(c_{ij})$，计算$\rho(C)$并取最大值即可得到$\rho(TG)$。我们绘制$(i,j)-\rho(C)$图，如下图所示，可以发现$\rho(TG)<0.25$。
...
下面我们取$n=2^{5},...,2^{10}$的网格测试，方程组求解误差达到$10^{-8}$即停，记录各实验组达到理论误差所需的迭代次数。理论误差、迭代次数与求解时间如下表（误差均以无穷范数计）。
...
考虑到本文通篇采用的第一人称是“我们”，这里做出解释：“我们”指的是作者Wenchong Huang和他5岁的妹妹。
...
另外，我们做了二维规则情况下的一些理论分析。不过不规则情况下的分析过于复杂，由于我们采用了扭曲网格，导致Laplace方程变形，使得理论分析极其困难。针对扭曲网格的理论分析，我们认为一种可行的思路是借助不确定性量化中的一些方法，因为和规则网格相比，Laplace方程的系数偏差大约在$\frac{0.01}{h^2}$，Neumann边界方程的系数偏差大约在$\frac{0.01}{h}$，也许我们可以不关心偏差的具体数值，将其视为一种“扰动”来分析。

总之，我们对实验结果感到满意，也希望能够进一步完善这份实验。
