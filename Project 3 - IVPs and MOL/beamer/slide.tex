...
\begin{frame}
  ...
  For the linear ODE
  \begin{equation*}
    \mathbf{u'}(t)=A\mathbf{u}+\mathbf{b}(t),
  \end{equation*}
  the stiffness ratio is
  \begin{equation*}
    \frac{\max\limits_{\lambda\in\Lambda(A)}|\text{Re}\;\lambda|}{\min\limits_{\lambda\in\Lambda(A)}|\text{Re}\;\lambda|}.
  \end{equation*}

  The stiff equation always have a large stiff ratio.
\end{frame}
...
\begin{frame}
  ...
  \vspace{1em}
  It constructs a $s$-stages, $(2s)$th-order collocation method, which is A-stable and B-stable but not L-stable. The order could be arbitary high without extra artifical work.
